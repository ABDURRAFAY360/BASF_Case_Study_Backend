...
class ReviewService:
    ...
    async def upsert(self, *, book_id: int, username: str, data: ReviewUpsertRequest) -> ReviewRead:
        if not await self.books.get(book_id):
            raise HTTPException(status_code=404, detail="Book not found")
        saved = await self.reviews.upsert(book_id=book_id, username=username,
    ...
    async def list_for_book(self, *, book_id: int) -> list[ReviewRead]:
        if not await self.books.get(book_id):
            raise HTTPException(status_code=404, detail="Book not found")
        return [ReviewRead.model_validate(x) for x in await self.reviews.for_book(book_id)]
